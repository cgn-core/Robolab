...
from robolab.configs import Hyperparameters, TestingParams
...
def evaluate(
    ...
    model = ConvNet(num_classes=Hyperparameters().num_classes)
    model.load_state_dict(
        torch.load(checkpoint_path, map_location=device, weights_only=True)
    )
    model.to(device, dtype=getattr(torch, TestingParams().dtype))
    model.eval()
    logger.info(f"Model loaded with {total_params(model):,} total parameters.")
    ...
    with torch.no_grad():
        for images, labels in test_loader:
            images = images.reshape(-1, 3, 32, 32).to(
                device, dtype=getattr(torch, TestingParams().dtype)
            )
